...
const departmentFormDirty = computed(() => { if (!state.formVisible.value) return false; const original = state.editing.value ? {name: state.editing.value.name, code: state.editing.value.code || ''} : {name: '', code: ''}; return state.form.name !== original.name || state.form.code !== original.code })

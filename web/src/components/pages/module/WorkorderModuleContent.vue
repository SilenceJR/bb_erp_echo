  ...
  <DataTableShell v-bind="tableState" aria-label="任务单列表" @retry="loadActiveModule" @update:page="handlePageChange" @update:page-size="handlePageSizeChange">
    <div class="responsive-table-desktop">
      <el-table :data="rows" row-key="id" stripe class="data-table">
        ...
        <el-table-column label="产品型号/数量" min-width="160"><template #default="{row}">{{ row.product_model || '-' }}<br><small>{{ formatQuantity(row.planned_quantity) }}</small></template></el-table-column>
        ...
        <el-table-column label="查看" width="100" fixed="right"><template #default="{row}"><el-button link type="primary" @click="openWorkOrder(row)">详情</el-button></template></el-table-column>

...
export function useStartupConnection() {
  ...
  async function useSavedServer(saved: ServerIdentity, expectedGeneration: number): Promise<boolean> {
    ...
    transition('Validating', '正在连接上次使用的服务器')
    try {
      ...
      await finishConnection(identity, expectedGeneration, '已连接上次使用的服务器')
      return true
    } catch (reasonValue) {
      if (expectedGeneration !== generation) return true
  ...
  async function startDesktop(expectedGeneration: number) {
    const saved = platform.connection.savedIdentity()
    if (desktopStartupTarget(saved) === 'saved-server' && saved && await useSavedServer(saved, expectedGeneration)) return
    if (expectedGeneration !== generation) return

    const savedValidationError = error.value
    transition(
      'Discovering',
      savedValidationError ? '无法连接上次使用的服务器，正在发现局域网服务' : '正在发现局域网内的博邦 ERP 服务',
    )
    try {
      ...
      if (savedValidationError) {
        transition('ManualSetup', '上次使用的服务器验证失败')
        error.value = savedValidationError
        return
      }

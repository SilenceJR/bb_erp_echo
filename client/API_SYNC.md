...
- 登录后通过 `GET /api/v1/notifications/stream` 建立带 Bearer 头的 fetch-SSE 长连接；禁止在 URL 传递 access token。Tauri 平台传输为该流禁用普通 8 秒超时，仍响应调用方取消。
- `data_changed` 事件使用版本 1 的 `module/items/action/refresh` 结构；客户端只显示白名单摘要。消息仅保留本次登录最近 20 条，无离线回放。
- 单实体可进入模块详情；批量、删除或无可靠定位的变更降级到模块列表。当前页面刷新不得覆盖脏表单。
- 通知断线不表示业务服务整体不可用；重连后重新同步当前安全模块。需要离线必达、已读回执或待办状态时，必须新增持久化任务机制，不得扩大本临时流的语义。

## 模具与原生拖放契约（2026-09-08）

- 位置仍通过 `location_id` 关联；`卡板` 为互斥的专用位置代码，无编号。货架在共享 UI 拆成区、排、列三个下拉，Excel 保持 `A1-1` 单列格式。默认货架 100 项加卡板 1 项。
- `POST /api/v1/mold-locations/bulk` 接收 `{zone, rows, columns}`，返回 `{created}`；区名为 1–8 个英文字母并转大写，两段范围各为 1–100。扩展只增加缺失位置，不删除、改 ID 或重新启用停用项，沿用模具写权限。
...
Windows 发布标签用于服务端版本和 Artifact 名称；客户端 EXE 及签名 `client-update.json` 的版本独立取自 `src-tauri/tauri.conf.json`。`package.json`/lock、Cargo manifest/lock 和 Tauri 配置必须保持同一客户端版本；CI 会检查更新清单与该源码版本一致。当前客户端版本为 `0.3.0`。

## 2026-09-05 Astra 整改验证边界

本轮不改变 API、权限、账号初始化和模块过渡契约。以下为历史验收记录；模具详情、新建和编辑已由 2026-09-08 的中央全宽规则替代。其他模块详情、新增和编辑共用面板载体；查看 420px，表单/图片 520px；停靠条件统一为视口至少 1440px 且扣除导航、面板、分隔及中央 48px 内边距后至少保留 720px。详情和表单采用同一套空间过渡、焦点恢复和未保存保护。Tauri 前端与 macOS 应用构建已通过；macOS原生启动、发现及进入登录页已验证，但已认证原生业务流、Windows及脱敏真实样本验收未完成。详见 [逐页验收记录](../docs/ASTRA_ACCEPTANCE.md)，不得以构建代替成品运行验收。
...
- 登录后共用三栏客户端壳：左侧业务导航支持完整/图标/隐藏，中央保留 ERP 页面，除模具使用中央全宽详情外，其他模块右侧详情在宽屏停靠、1280px 以下覆盖。外观状态仅保存在当前电脑的 `bb_erp_theme_mode`、`bb_erp_theme_accent`、`bb_erp_sidebar_mode`，不与账号同步。
...
- 权限清单不再作为独立导航页面；角色配置仍调用 `GET /api/v1/system/permissions` 获取完整选项，并通过 `POST /api/v1/system/roles/:id/permissions` 一次提交完整 `permission_ids`。全局和分类全选只改变本地草稿，不新增服务端批量授权捷径。

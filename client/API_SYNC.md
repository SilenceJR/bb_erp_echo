...
- 模具新导出使用根部扁平目录：单模目录为完整编号，共模目录为同组全部成员编号以 `+` 连接，图片与 DWG 直接混放。导入可剔除一层统一包装目录、解码 Windows GBK 文件名，仍兼容旧 `images/`、`drawings/` 资料包；平铺文件按完整编号识别，不猜型号别名。文件名含“产品刷墨图”或以 `-正整数` 结尾归产品图，其他图片归模具图；内部分类值仍为 `product_material` / `supplement`。未匹配及含 `+` 单模/共模歧义资料通过预览项的 `kind`、`allowed_codes` 选择归属，提交端点和 `corrections` 机制不变。

...
- 客户编码、客户资料（含单组联系人信息）和供应商档案；客户支持 `.xls/.xlsx` 原子导入、工作表预览及 `.xlsx` 导出。
...
客户模块仅面向全新 SQLite 数据库：不检测、转换或清理旧客户/联系人表。客户编码默认按 `BB-001` 递增并可关联多条具体客户资料；库存、任务单和模具选择具体资料 ID，统计中的客户总数按客户编码计数。

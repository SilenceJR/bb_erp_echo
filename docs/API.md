...
`molds.xlsx` 列为：序号、模具编号、模具型号、模具类型、模具位置、共模组号、图片总数、备注。模具位置使用 `A1-1` 等字符串，卡板写作 `卡板`；序号和图片总数在导入时忽略并重新计算。单模目录使用完整模具编号；共模目录使用同一共模组的全部成员编号以 `+` 连接，成员顺序不限。目录内直接混放图片与 DWG/FDWG；共模文件通过完整编号关联成员，不猜测型号别名，无法识别的图片或图纸进入待修正列表。若含 `+` 的单模编号与共模目录同名，预览返回允许选择的候选编号，由用户逐项确认归属；同一目录内不能混合选择单模解释和共模解释。待修正项返回 `kind` 和 `allowed_codes`，提交仍使用原有 `corrections` 中的 `codes` 与图片内部分类值。文件名去除扩展名后，包含“产品刷墨图”或以 `-正整数` 结尾时归为产品图，其他合法图片归为模具图。API 分组值仍为 `product_material`、`supplement`。允许 ZIP 外套一层统一包装目录；Windows 旧压缩包的 GBK 文件名会转换为 UTF-8；忽略 `desktop.ini`、`Thumbs.db`、`.DS_Store` 和 `__MACOSX`。旧 `images/`、`drawings/` 资料包在过渡期继续可导入，新导出只生成扁平目录。

导出时，普通产品图按自然顺序命名为 `<模具编号>-1.ext`、`-2.ext`……；原名含“产品刷墨图”的产品图保留该业务标识。模具图与图纸尽量保留有意义原名；共模成员专属资料确保只带该成员编号，共用资料带全组编号前缀。同组同分类、同原名且内容一致的全组资料只输出一次。发生重名时使用稳定的“副本 + 记录 ID”后缀，且保证回导后图片分类和成员归属不变。

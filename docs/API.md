...
`molds.xlsx` 列为：序号、模具编号、模具型号、模具类型、模具位置、共模组号、图片总数、备注。模具位置使用 `A1-1` 等字符串，卡板写作 `卡板`；序号和图片总数在导入时忽略并重新计算。新扁平结构的单模目录使用完整模具型号；共模目录使用同一共模组的全部成员型号以 `+` 连接，成员顺序不限。目录内直接混放图片与 DWG/FDWG；共模文件通过完整型号关联成员，不猜测拼写或别名，无法识别的图片或图纸进入待修正列表。若含 `+` 的单个模具型号与共模目录同名，预览返回允许选择的候选模具编号，由用户逐项确认归属；同一目录内不能混合选择单模解释和共模解释。待修正项返回 `kind` 和 `allowed_codes`，提交仍使用原有 `corrections` 中的内部模具编号 `codes` 与图片分类值。文件名去除扩展名后，包含“产品刷墨图”或以 `-正整数` 结尾时归为产品图，其他合法图片归为模具图。API 分组值仍为 `product_material`、`supplement`。允许 ZIP 外套一层统一包装目录；ZIP 中合法 UTF-8 文件名直接保留，仅对无效 UTF-8 字节尝试 Windows GBK 转换；忽略 `desktop.ini`、`Thumbs.db`、`.DS_Store` 和 `__MACOSX`。旧 `images/`、`drawings/` 资料包在过渡期继续按模具编号导入，新导出只生成按模具型号组织的扁平目录。

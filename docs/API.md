...
`molds.xlsx` 列为：序号、模具编号、模具型号、模具类型、模具位置、共模组号、图片总数、备注；模具位置仍使用原字符串格式（例如 `A1-1`），卡板写作 `卡板`。模板包含一个单模示例和两个同属 `TEMPLATE-GROUP` 的共模示例，并附默认 100 个货架位置及 `卡板`；用户可以替换示例行、按目录添加自己的图片与 DWG 文件。导入忽略序号和图片总数，重新生成 ID 并计算图片数量；没有 `locations.json` 时使用同一默认位置生成规则，旧资料包显式提供位置时保留其自定义、停用状态和关联，只补充缺失的 `卡板`。标准单模目录仍为 `images/<模具编号>/<category>/` 与 `drawings/<模具编号>/`；共模组外层目录使用成员编号连接，组内完全相同的同名同类图片/图纸放在 `共用/`，其他文件放在对应具体编号目录，避免丢图或串图。导入同时兼容旧目录、新共模目录和 `images/<共模组>/<文件>`、`drawings/<共模组>/<文件>` 平铺文件：平铺文件按最长且不重叠的完整模具编号匹配（例如存在 `AB` 与 `AB-CD` 时，`AB-CD-1.png` 仅归属后者），不猜测别名（例如 `FSL2214` 不会当作 `FL2214`）；无法识别的图片进入待修正列表，无法匹配的图纸直接报错。`产品材料`、`产品图`、`材质`归入产品材料，`前模`、`后模`、`开模`、`尺寸`、`局部`、`原理图`归入补充图；能精确匹配编号但未包含分类关键词的平铺图片按产品材料处理。文件名中的 `+` 仍可表示多个编号，旧任意自定义模具编号可继续回读。未修正、未匹配、重复或扩展名不受支持的文件不能提交。资料包提交时复用图库的真实内容解码和静态预览流程，因此系统导出的 JPG/JFIF、PNG、GIF、WebP、HEIC/HEIF、AVIF、BMP、TIFF、SVG 图片可以按相同安全边界回导。

导入采用“预览—修正—确认—提交”，预览阶段即按图库规则解码真实图片内容。ZIP 文件句柄由请求持有至附件暂存结束后关闭，避免预览后提交无法读取附件。资料包最多 2000 个源条目，共模复制后最多落盘 5000 个图片/图纸；声明解压总量和提交时实际写入总量均不得超过 4 GiB，单个 ZIP 请求仍不得超过 2 GiB；`molds.xlsx`、`locations.json` 和人工修正参数另有 64/4/4 MiB 小对象边界。提交在事务内使用明确的全量删除条件替换模具、DWG 和位置字典，图片仅替换 `owner_type=mold` 的记录，不影响客户、用户、库存和工单；模具图片、图纸写入/删除、资料包生成与全量导入使用同一资产互斥边界（ZIP 完成后先释放锁，再发送下载），旧物理文件在事务提交后清理，失败时进入启动重试任务。Excel 单独导出不包含图片；`.xls` 内嵌图片不在本期自动提取范围内。

...
`0.0.2`、`0.0.3` 和 `0.0.4` 的 Windows 构建均已完成，但 Gitee 大附件接口持续出现 0 字节响应超时。`0.0.5` 已建立完整更新基线，`0.0.6` 已完成相邻版本发布；`0.0.7` 已修复公钥部署边界并通过七个正式附件、匿名 SHA-256、签名差分基线和稳定 manifest 技术验收。版本之间不改变 Web/Tauri 业务 API，Windows 真机的手动恢复、差分或完整包回退、重启和数据保留仍待用户验收。

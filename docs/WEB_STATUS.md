...
`0.0.2`、`0.0.3` 和 `0.0.4` 的 Windows 构建均已完成，但 Gitee 大附件接口持续出现 0 字节响应超时。`0.0.5` 已建立完整更新基线，`0.0.6` 已发布七个正式附件并通过匿名 SHA-256、签名差分基线和稳定 manifest 验收。版本之间不改变 Web/Tauri 业务 API，Windows 真机的差分或完整包回退、重启和数据保留仍待用户验收。
...
- `0.0.7` 将通过启动脚本锁定包内公钥、服务端无效直接值回退和升级器替换发布文件修复。由于旧版检查链路已在验签入口失败，需手动部署 `0.0.7` 全量包一次。
- `0.0.7` 成功读取稳定 manifest 后，使用 `0.0.8` 验收检查状态、差分/完整包回退、重启版本显示和数据保留。当前只记录故障与修复范围，不将未运行的 Windows 结果标记为已完成。

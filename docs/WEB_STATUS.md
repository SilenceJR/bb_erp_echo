...
- 新增独立“产品资料”页面，支持产品型号、客户型号、产品材料可输入下拉、是否刷墨、状态、单一图片组和关联模具抽屉。
- 模具页面已移除模具业务编码，改为产品型号、模具类型、模穴数、位置、共模组号、单一模具图片组和 DWG/FDWG。
- 仓库页面改为产品数量与库位统计，数量操作为入库、出库、库位调整和盘点修正；不再展示产品/物料建档、成本、安全库存、供应商和客户字段。
- 生产任务从产品资料选择启用产品，展示产品型号与实时数量；临时产品建档入口已删除。
- 已验证：Web 77 项测试、Web 与 Client 生产构建。真实浏览器权限矩阵、ZIP 往返、`1024×680` 至 `1920`、暗色主题和 Windows/WebView2 仍待运行态验收。

## 实时通知与管理详情（2026-09-09）

- 共享 Vue 工作台已接入 Bearer fetch-SSE；Tauri 长连接不使用普通业务请求的 8 秒超时。断线重连后只刷新可安全替换的当前页面。
- 右上角一次显示一张 8 秒提醒卡，顶栏铃铛保留当次登录最近 20 条和 `99+` 未读计数；消息不持久化，退出或刷新即清空。
- 当前模块可安全刷新时静默同步且不留未读；创建、编辑、提交或脏表单状态不被覆盖，改为提示外部更新。未支持精确定位的页面安全降级到模块列表。
- 账号详情按账户类型互斥展示：个人账号显示当前角色，部门终端账号显示角色合并后的当前权限。审计列表将稳定动作编码转为具体中文操作。
- 图片大图预览存在时，Escape 先由预览层关闭，不穿透关闭背后详情。Web 77 项测试、类型检查与 Web/Client 生产构建已通过；真实 Tauri/Windows 按键序列待验证。

## 模具优化工作项（2026-09-08）

- 模具位置以“货架 / 卡板”互斥选择，货架使用三级联动下拉；位置管理按区创建或扩大范围，导入导出保持位置字符串。
- 模具详情、创建和编辑使用中央全宽页面，共模仍每模具一行。产品图采用一张主图与可翻页缩略图，模具图使用等尺寸网格；内部 `product_material` / `supplement` 值保持不变。
- 模具资料包引导已改为根部扁平单模/共模目录，并明确“有目录覆盖、无目录保留、Excel 删除即删除资料”的提交边界。
- 详情删除直接使用红色按钮，确认中明确编号、附件范围及不可恢复；查看态不再显示遮挡图片的重复底栏。位置筛选作为完整横向字段，空间不足时独立换行。
- 图片、图纸、模具 ZIP 和客户 Excel 共用原生拖放入口，修复 SVG 命中、旧区域高亮与身份续期缺失；忙态、无权限、文件类型和单文件数量校验保持一致。
- 浏览器、Windows/Tauri 与独立审查验收结果以本次 `BACKEND_STATUS.md` 记录为准；尚未执行的运行层级不标记为通过。
...
| 单 EXE 客户端更新 | 已完成代码，待 Windows 真机验收 | 连接验证后后台检查一次；登录前及设置可手动检查。仅支持内网 protocol v1 portable EXE，签名/哈希/大小二次校验、原子替换、启动确认与回滚；Web 不显示更新中心 |
...
| 模具重构 | 已完成，待真实文件运行验收 | 页面支持筛选、逐行表格与全宽详情、产品图/模具图、DWG 文件、位置管理、跨页批量移位、ZIP 预览/人工修正/导入导出，以及 ZIP、图片、DWG 拖拽添加；模具模板为含示例行和扁平单模/共模目录的 ZIP，客户 XLSX 与模具 ZIP 的下载/导入/导出固定契约集中在 `transferRegistry`；不迁移旧生命周期 UI |
...
- 本轮模具交互：图片组、ZIP 导入和 DWG 区域增加键盘可达的拖拽/点击入口；客户 Excel 导入同样提供拖放区，顶部按钮只打开弹窗，点击区域才打开系统文件选择器；Tauri 原生文件拖放路径由 Rust 直接流式上传，Windows 不依赖 HTML5 `dataTransfer.files`；详情加载失败、未保存关闭、编辑入口和窄屏 Drawer/空态显示已整改；Web/Tauri 生产构建与真实 Finder/资源管理器拖入仍需运行态验收。
...
- 标题栏关闭、登出、切服、更新安装时的真实脏表单保护，以及客户端单 EXE 下载、替换、启动确认、目录不可写和回滚。

...
本轮已重建公共控件、连续工作面及统一详情/新增/编辑面板，Web 类型检查、生产构建和原 27 项测试通过；终审增加 3 项状态回归，当前 30/30 测试通过。隔离合成样本库下完成 15 页尺寸/溢出检查及指定面板、草稿、焦点验证；这不是脱敏真实样本或 Windows 成品验收。最新逐页状态、证据和未验收项以 [Astra 验收记录](ASTRA_ACCEPTANCE.md) 为准。下表历史“已完成”表示实现状态，不代表本轮全部功能已经运行验收；产品终审三项P1已修复并通过限定范围复验，整套成品尚未完成验收，未发布。
...
| CSS/Motion 分层 | 已完成，本地浏览器已复验 | 高频 hover/focus/颜色反馈使用 CSS；停靠详情用 180ms 网格与最多 6px 轻位移形成空间连续性，正文首帧可读，不使用整页透明入场；覆盖式 Drawer 由 Element Plus 管理遮罩、焦点和开闭；全局尊重 reduced motion |

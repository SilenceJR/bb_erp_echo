...
- 模具详情、创建和编辑改为中央全宽页面，其他模块详情不变；共模仍每模具一行。产品图采用一张主图与最多三张可切换缩略图，更多图片可翻页；补充图使用等尺寸网格，模具图均完整显示。
...
| 模具重构 | 已完成，待真实文件运行验收 | 页面支持筛选、逐行表格与全宽详情、图片分组、DWG 文件、位置管理、跨页批量移位、ZIP 预览/人工修正/导入导出，以及 ZIP、图片、DWG 拖拽添加；模具模板为含示例行和标准空目录的 ZIP，客户 XLSX 与模具 ZIP 的下载/导入/导出固定契约集中在 `transferRegistry`；不迁移旧生命周期 UI |

...
`0.0.2`、`0.0.3` 和 `0.0.4` 的 Windows 构建均已完成，但 Gitee 大附件接口持续出现 0 字节响应超时。当前以 `0.0.5` 建立可恢复上传的正式基线，再以 `0.0.6` 验证相邻版本更新；版本之间不改变 Web/Tauri 业务 API。验收重点是客户端版本注入、正式 manifest、差分或完整包回退、重启和数据保留。

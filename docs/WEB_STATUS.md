...
`0.0.2`、`0.0.3` 和 `0.0.4` 的 Windows 构建均已完成，但 Gitee 大附件接口持续出现 0 字节响应超时。`0.0.5` 已通过六个正式附件、匿名 SHA-256 和稳定 manifest 验收，作为完整更新基线；下一步以 `0.0.6` 验证相邻版本更新。版本之间不改变 Web/Tauri 业务 API，Windows 真机的差分或完整包回退、重启和数据保留仍待用户验收。

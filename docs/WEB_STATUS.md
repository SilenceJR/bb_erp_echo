...
本次正式发布原计划使用当前代码生成 `0.0.2`，但 Gitee 附件上传两次长时间无响应；源码标签未改写，当前改以包含发布脚本修复的 `0.0.3` 作为兜底候选，再以发布验收文档增量生成 `0.0.4`。两版之间不改变 Web/Tauri 业务 API；验收重点是客户端版本注入、正式 manifest、差分或完整包回退、重启和数据保留。

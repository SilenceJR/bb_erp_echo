...
本次正式发布原计划使用当前代码生成 `0.0.2`，但 Gitee 大附件上传两次长时间无响应；源码标签未改写，`0.0.3` 的并行上传仍因大附件 0 字节响应超时失败，当前改以包含 multipart 表单认证修复的 `0.0.4` 作为第二轮兜底候选。版本之间不改变 Web/Tauri 业务 API；验收重点是客户端版本注入、正式 manifest、差分或完整包回退、重启和数据保留。

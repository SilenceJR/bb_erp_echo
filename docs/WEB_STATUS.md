...
本次正式发布原计划使用当前代码生成 `0.0.2`，但 Gitee 大附件上传两次长时间无响应；源码标签未改写，`0.0.3` 的并行上传仍因大附件 0 字节响应超时失败。`0.0.4` 的 Windows 构建和包生成成功，但发布作业先因多行 Artifact `pattern`、后因运行号通配及全量合并漏掉直接上传文件而在上传前失败，现已改为按实际 Artifact 名称逐个下载，并支持已有正式标签安全重试。版本之间不改变 Web/Tauri 业务 API；验收重点是客户端版本注入、正式 manifest、差分或完整包回退、重启和数据保留。

...
本轮以 Astra 设计覆盖启动、连接、登录、全部模块与公共交互，不能以原版测试通过代替新版产品验收。首页静态关注、入口数指标和重复导航已移除；统计无效搜索已移除，来源不可用指标和空分布面板不再填充展示。此次 Web 类型检查和生产构建通过；当前测试 25/27，两项暗色按钮/停靠动画样式断言尚未通过，需要随完整组件改造检查实现与断言。完整运行态与原生产品验收未完成，未提交发布。

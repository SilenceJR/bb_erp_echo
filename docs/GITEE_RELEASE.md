...
- 修复提交首次镜像产生的 GitHub Actions #77 在作业创建前报 `(Line: 308, Col: 14): Exceeded max expression length 21000`。原因是新增打包逻辑使含 `${{ ... }}` 的单个 PowerShell `run` 标量超过平台上限；现已把版本、标签、更新地址及 Gitee 仓库变量改为步骤 `env` 输入，避免 GitHub 把整段脚本作为超长表达式标量处理。修正后的 [GitHub Actions #78](https://github.com/SilenceJR/bb_erp_echo/actions/runs/33237255735) 已成功创建并完成 Go、Web、Tauri 前端与 Rust 核心四个验证作业；Windows 正式打包仍须由后续正式标签验证。

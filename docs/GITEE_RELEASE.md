...
本次正式发布采用当前 `main` 代码作为 `0.0.2` 基线，保留远程既有 `v0.0.1` 标签及其提交历史，不覆盖已有远程对象。随后仅增加发布验收文档提交并发布 `0.0.3`，用于验证相邻正式版本的客户端升级。

由于公开发布仓库仍存在历史 `0.1.0-rc.3` 稳定清单，发布脚本增加了严格的一次性迁移条件：只有目标版本为 `0.0.2`、当前稳定版本精确为 `0.1.0-rc.3`，且 CI 显式传入迁移标志时才允许覆盖稳定清单；其他版本继续执行正常的 SemVer 递增保护。历史 RC Release 和源码提交保留不变。

发布顺序固定为：

1. 提交发布脚本、CI 和文档变更，使用中文提交信息 `发布：准备正式版本 0.0.2`。
2. 推送 Gitee `main` 和 `v0.0.2`，等待 GitHub 镜像同步；确认 Go、Web、Tauri、Windows 构建、签名、Release 附件匿名下载、大小、SHA-256 和稳定 manifest 校验全部通过。
3. 记录 `0.0.2` 的 CI 与 Release 验收结果，使用中文提交信息 `发布：记录 0.0.2 验收并准备 0.0.3`。
4. 推送 `v0.0.3`；确认其差分基线版本和 Portable EXE 哈希来自 `0.0.2`。差分包不满足体积阈值时，必须验证完整签名包回退。

`0.0.3` 不新增业务 API 或功能代码，仅通过标签注入生成不同版本的正式客户端，用于验证 `0.0.2 → 0.0.3` 更新、重启和失败恢复。用户应从 `0.0.2` 全量包开始测试，并确认数据库、上传图片、配置和日志未被升级覆盖。

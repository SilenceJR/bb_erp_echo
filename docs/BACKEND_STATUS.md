...
- 本轮代码已实现：模具模板和新导出改为按模具型号组织的根部扁平单模/共模目录，图片与 DWG 直接混放；可导入额外套一层包装目录的资料包，并兼容旧按模具编号识别的 `images/`、`drawings/` 格式。
...
- 代码已实现：共模导出按组创建一个以全部成员型号连接的扁平目录，图片与图纸直接混放；全组同名同类同内容文件只导出一份，成员专属资料保留唯一归属。旧 `images/`、`drawings/` 目录按模具编号兼容，新扁平目录按完整模具型号识别，不猜别名。列表仍一模具一行，共用文件回导到各成员自己的资料记录。

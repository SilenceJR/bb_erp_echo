...
## 产品型号与业务链重构（2026-09-10）

- 已实现产品资料独立主档：`product_model` 唯一，支持客户型号、产品材料、是否刷墨、启用/停用、产品图片、产品 ZIP 模板/预览/提交/导出。
- 已实现模具去业务编码：模具通过产品 ID 关联产品资料，API 固定返回 `product_model`，保留单/共模、模穴数、位置、共模组号、单一模具图库和 DWG/FDWG；ZIP 使用 `<产品型号>/<三位序号>/`，缺产品型号时自动创建启用占位产品。
- 已实现仓库纯数量模式：仅产品数量与库位，支持入库、出库、库位调整、盘点修正；物料、成本、安全库存、供应商和客户字段不在新页面和接口中。
- 已实现生产任务关联产品资料并保存产品型号快照，删除临时产品建档接口和权限；统计报表按产品型号、库位、模具类型和位置聚合。
- 已验证：`go test ./...`、`go vet ./...`、产品/模具/仓库/任务专项测试、Web 77 项测试、Web 与 Client 生产构建、Tauri 26 项 Rust 测试。
- 已用独立临时 SQLite 和真实 HTTP 请求验证：登录、产品建档、模具关联并返回产品型号、产品库存入库与流水、生产单产品型号快照、产品 ZIP（含图片）往返和模具 ZIP（含图片与 DWG）往返。真实浏览器页面流程、Windows/Tauri 和完整权限矩阵仍待验收。
- 新结构只面向全新数据库；不提供旧库迁移、兼容或清库代码，旧库需人工删除重建。
...
- 发布后修复中：Windows 原生拖放上传已补齐嵌套 `request.server_url`；macOS ZIP 的合法 UTF-8 文件名不再错误按 GBK 二次解码；手工编辑工作簿省略空“序号”单元格时会按模具类型列恢复对齐；新扁平资料目录按产品型号解析并映射回内部产品型号。
- 本轮代码已实现：模具模板和导入导出统一为按产品型号组织的根部扁平单模/共模目录，图片与 DWG 直接混放；可导入额外套一层包装目录的资料包，旧 `images/`、`drawings/` 格式不再兼容。
- 本轮代码已实现：文件名含“产品刷墨图”或以 `-正整数` 结尾的图片归产品图，其他合法图片归模具图；界面名称已同步，内部分类值保持不变。系统元数据文件被忽略，Windows 样本 ZIP 的 GBK 中文文件名可正常解码。
- 本轮后续适配：共模文件优先按完整型号匹配，并支持唯一忽略 `-` 以及字母前缀与数字之间的单个 `S` 工艺变体；冲突候选仍进入人工确认。人工修正通过 `allowed_molds` 显示产品型号、内部继续提交产品型号；未命中特殊产品图规则的图片默认选择“模具图”。
...
- 代码已实现：共模导出按组创建一个以全部成员型号连接的扁平目录，图片与图纸直接混放；全组同名同类同内容文件只导出一份，成员专属资料保留唯一归属。导入仅按完整产品型号识别根部扁平目录，不猜别名。列表仍一模具一行，共用文件回导到各成员自己的资料记录。
...
- 模具资料包导入使用预览令牌和事务更新，仅清理模具、模具图片、预览文件、DWG 与位置字典；模板和正式导出均为 ZIP，包含单模、共模示例行、默认位置以及根部扁平资料目录，旧 `images/`、`drawings/` 结构不再接受。下载响应统一使用安全的 UTF-8 `Content-Disposition`、`no-store` 和 `nosniff` 头；最多 2000 个源条目、共模复制后 5000 个资产，声明解压总量与实际落盘总量均限制为 4 GiB，工作簿/位置/修正参数另有 64/4/4 MiB 边界。模具档案和位置按 Excel 全量更新；ZIP 中有目录的模具覆盖资料、无目录的同编号模具迁移并保留旧资料、Excel 删除的模具及资料一并删除。导入忽略 Excel 的 ID/图片总数，图片在界面按产品图/模具图分组，未知图片可在预览中人工指定内部分类和产品型号。资料包图片与图库共用扩展格式、预览阶段真实解码和静态预览规则，大小写扩展名均可识别，新格式可回导；图片、DWG、整模删除和全量导入使用同一资产互斥边界。
- API 只保留当前 canonical 路径：任务单 `/api/v1/workorder`、物料 `/api/v1/materials`、产品 `/api/v1/products`、模具 `/api/v1/molds`、仓库管理 `/api/v1/warehouses`，以及当前 `/api/v1/warehouse/products`、`/api/v1/locations` 路径；不注册旧任务、单数基础资料、`/api/v1/inventory` 或 `/api/v1/warehouse` 根别名。图片权限只校验当前业务对象权限。

...
- `v0.2.8` 发布准备：模具关系图 ZIP 按型号解析、中文文件名和 Windows 原生拖放修复已完成，旧 `images/`、`drawings/` 导入兼容已移除；Tauri、npm/Cargo 清单及锁文件客户端版本已统一为 `0.2.8`。真实 Windows 构建、签名、发布产物和目标机验收仍由标签 CI 与真机验证确认。

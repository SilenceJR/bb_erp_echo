...
运行 `启动服务端.vbs` 或完整包的 `启动系统.vbs` 后，服务端不显示控制台窗口，而是常驻 Windows 右下角通知区。只有数据库、HTTP 和局域网发现服务全部就绪后，才会提示“博邦服务已启用”。

右键托盘图标可以查看全部有效局域网 IPv4 访问地址、打开管理页面、检查状态、重启服务、打开日志目录或退出服务。退出前会二次确认；退出后所有已连接客户端都将无法继续使用。
...
全新数据库可直接启动；如需额外的系统托管管理员，可在首次启动前配置其密码。正式使用也建议通过环境变量覆盖其他敏感配置：
...
- `BB_ERP_SILENCE_PASSWORD` 只用于全新数据库首次初始化额外的 `Silence` 超级管理员；源码不提供默认值。未配置时不创建该额外账号，也不阻断启动；已有数据库不会补建或重置该账号。
...
在全新数据库首次启动前配置 `BB_ERP_SILENCE_PASSWORD` 时，系统还会额外创建独立的 `Silence` 超级管理员；未配置时直接跳过该额外账号。该账号不会替代或修改原有 `admin`；已有数据库升级时也不会自动补建。它是系统托管的隐藏保底账号，不显示在“用户账号”管理列表中，管理员也不能通过用户管理接口修改它；请使用 Silence 本人登录后的密码修改入口维护凭据。
...
- 有 `mold:import` 权限时，在“导入模具资料包”中下载 `博邦模具导入模板.zip`。根目录含 `molds.xlsx`、`locations.json` 和扁平模具目录：单模用完整编号，共模用同组全部编号以 `+` 连接，图片和 DWG 直接混放。
- 文件名含“产品刷墨图”或以 `-正整数` 结尾的图片归为产品图，其他图片归为模具图；编号别名不自动猜测，预览时需人工确认未识别图片或图纸的归属，图片还需明确选择产品图或模具图。

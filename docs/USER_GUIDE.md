...
BB_ERP_JWT_SECRET=请换成足够长的随机密钥
BB_ERP_ADMIN_USERNAME=admin
BB_ERP_ADMIN_PASSWORD=请换成正式管理员密码
```
...
1. 修改正式管理员密码。
2. 在“系统设置 / 部门”创建实际部门。
3. 在“系统设置 / 终端”创建车间公共电脑或部门终端。
4. 在“系统设置 / 用户账号”创建员工个人账号或部门终端账号。
5. 在“系统设置 / 角色”配置权限，再把角色分配给用户。

账号类型说明：

- 个人账号：给具体员工使用，审计日志记录具体人员。
- 部门终端账号：给车间公共电脑使用，审计日志记录部门和终端，具体人员显示为未知。
...
- 用户账号：创建个人账号和部门终端账号，启用/停用，重置密码，分配角色。
- 部门：维护工厂部门基础资料。
- 终端：维护公共电脑和部门终端。
- 角色：创建角色，并给角色配置权限。
- 权限：查看系统权限清单。
...
1. 办公室创建任务单，选择目标部门。
2. 办公室派发，系统为每个目标部门生成子任务。
3. 部门开始处理，可填写部分完成数量。
4. 部门完成本部门任务。
5. 所有部门完成后，主任务进入“待办公室确认”。
6. 办公室确认正常完成；特殊情况可填写原因强制完成。
...
- `department_terminal_operator`：部门终端操作员，默认拥有任务读写、旧任务兼容读写和库存查看。

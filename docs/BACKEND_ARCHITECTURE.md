...
flowchart TB
    ...
    subgraph service["Go ERP 服务（统一业务入口）"]
        ...
        domains["业务功能<br/>客户编码/客户资料、供应商<br/>仓库、物料、产品<br/>库存、模具、任务单<br/>统计、图片、版本更新"]:::done
...
| 基础资料 | 客户编码/客户资料（资料内单组联系人）、供应商、仓库和物品信息 | 已实现 |
...
权限模块只向业务层暴露统一 `Authorizer` 接口。`PolicyProvider` 从数据库的
完整角色关系和权限关系读取数据，在临时 Casbin 引擎中构建整份策略；所有
策略添加成功后，再通过一次原子指针替换当前快照。HTTP 权限中间件、文件
权限 handler、App 路由注入和角色服务都只调用这个 provider，不直接持有或
修改当前 Casbin 引擎。

因此策略刷新期间正在处理的请求继续使用旧快照，刷新完成后的请求使用新
快照，不会观察到 `ClearPolicy` 与逐条添加之间的空策略或半成品状态。数据库
读取、快照构建或策略写入失败时不会切换快照，既有权限保持可用，并由调用
方处理刷新错误。
...
sequenceDiagram
    ...
    participant auth as 认证与权限快照 provider
...
sequenceDiagram
    ...
    api->>check: 检查 owner、权限、单批 100 张、真实图片内容和转换安全边界
    alt 全部检查通过
        check->>files: 保留原图并生成静态 JPEG 预览
        check->>db: 在一个事务中保存全部图片记录
        db-->>api: 返回图片元数据数组
        api-->>page: 返回成功结果
        page-->>user: 刷新图库并显示上传数量
    else 任一图片失败
        check->>files: 清理本批次已写入的原图和预览
...
| 修改数据库模型或字段 | 数据层说明和受影响时序图 | 新库 schema、索引、软删除、默认组织/仓库和恢复方案；不添加旧库迁移或兼容分支。 |
| 修改权限或数据范围 | 公共能力层和相关业务图 | 页面隐藏不是最终授权；后端必须继续校验角色、组织、部门和 cost:view。 |
| 修改金额、库存数量或成本 | 库存时序图和业务说明 | 数量固定精度、金额单位、加权平均成本和无成本权限时的字段裁剪。 |
| 修改图片文件流程 | 文件能力和相关业务时序图 | owner 权限继承、路径安全、批量与转换资源边界、原图/预览生命周期；大文件路由在权限校验后应用专用体积和两小时处理边界，普通接口保持 32 MiB 全局边界；替换在一个事务内条件删除旧记录并创建新记录，删除先提交数据库状态，再清理物理文件；失败路径写入持久化清理任务并在服务启动时重试，避免可见记录指向已删除文件。 |
| 修改更新检查或发布流程 | 更新时序图和发布文档 | 公钥/私钥边界、签名和哈希校验、完整包校验与失败恢复。 |
